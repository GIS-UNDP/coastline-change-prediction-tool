...
def get_closest_datapoint(dates, dates_ts, values_ts):
    ...
    if dates[0].date()  < dates_ts[0].date() or dates[-1].date()  > dates_ts[-1].date(): 
        raise Exception('Time-series do not cover the range of your input dates')
    ...
    for i,date in enumerate(dates):
        ...
        temp.append(values_ts[find(min(item for item in dates_ts if item.date() > date.date()), dates_ts)])
