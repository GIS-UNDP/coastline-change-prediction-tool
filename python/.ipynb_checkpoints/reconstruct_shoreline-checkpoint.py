...
def keep_coords(coords,dates_pred,output,save_corrections):
    ...
    for n in range(len(dates_pred)):
        if dates_pred[n].date()>=output['dates'][-1].date() and dates_pred[n].date().month==1 and not(save_corrections):
            tmp=[]
...
def reconstruct_shoreline(time_series,transects,dates_pred,output,inputs,settings,n_steps_further,estimate_pop=False,save_corrections=False,geomtype='lines'):
    ...
    if plot:
        ...
        for i in range(len(output['shorelines'])):
            ...
            date = output['dates'][i]
            color=matplotlib.cm.jet(i/(len(output['shorelines'])+n_steps_further+1))
        ...
        for i,date in enumerate(list(coords_interp.keys())):
            ...
            x = x[ind]
            y = np.sort(y)
            color=matplotlib.cm.jet((i+len(output['shorelines']))/(n_steps_further+1+len(output['shorelines'])))
